...
public class LeaveRequestController {
	...
	public void initialize() {
		...
		// 시작 날짜 DatePicker에 날짜 제한 설정 적용
		leaveStartDate.setDayCellFactory(dayCellFactory);

		// 시작 날짜가 변경되었을 때 종료 날짜 DatePicker의 날짜 제한 설정
		leaveStartDate.valueProperty().addListener((observable, oldValue, newValue) -> {
			if (newValue != null) {
				// 종료 날짜 선택을 시작 날짜 이후로만 가능하게 설정
				leaveEndDate.setDayCellFactory(getEndDateCellFactory(newValue));
			}
		});

		// 종료 날짜도 기본적으로 현재 날짜 이후로만 선택할 수 있게 설정
		leaveEndDate.setDayCellFactory(dayCellFactory);
	}
	
	// 종료 날짜가 시작 날짜 이후로만 선택되도록 제한하는 팩토리
    private Callback<DatePicker, DateCell> getEndDateCellFactory(LocalDate startDate) {
                ...
                return new DateCell() {
                    ...
                    public void updateItem(LocalDate item, boolean empty) {
                        ...
                        if (item.isBefore(startDate)) {
                            setDisable(true);
                            setStyle("-fx-background-color: #ffc0cb;"); // 비활성화된 날짜 시각적 구분
                        }
                    }
                };
            }
	...
	private void sendLeaveRequest(LocalDate startDate, LocalDate endDate) throws IOException {
		...
		ForRequestLeaveDto forRequestLeaveDto = new ForRequestLeaveDto.Builder().requestDate(LocalDate.now()) // 신청일을 현재
																												// 날짜로
																												// 설정
		...
		String jsonSendStr = communicationUtils.objectToJson(MessageTypeConst.MESSAGE_LEAVE_REQUEST,
				forRequestLeaveDto);
		...
		try {
			...
			// 서버로부터 응답 처리
			ResponseData<ForRequestLeaveDto> responseData = communicationUtils.jsonToResponseData(jsonReceivedStr,
					ForRequestLeaveDto.class);

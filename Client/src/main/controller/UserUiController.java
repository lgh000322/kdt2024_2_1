...
public class UserUiController implements Initializable {
   ...
   @FXML
   private TableView<WorkRecord> tableView;
   ...
   /* 휴가신청탭 테이블뷰 컬럼*/
   @FXML
   private TableColumn<LeaveRecord, Integer> leaveNoColumn;
   @FXML
   ...
   @FXML
   private TableColumn<LeaveRecord, String> leaveAcceptColumn;
...
//   private TableColumn<QnARecord, String> qnaDateColumn;
      ...
      try {
         ...
         if (messageType.contains("성공")) {
            ...
            for (int i = 0; i < list.size(); i++) {
               ...
               WorkRecord workRecord = new WorkRecord(userWorkData.getWorkNum(), userWorkData.getWorkDate(),
                     userWorkData.getStatus(), userWorkData.getStartTime(), userWorkData.getEndTime(), "");
               workRecordList.add(workRecord);
            }
            
            tableView.setItems(workRecordList);
         }
      } catch (IOException e) {
         e.printStackTrace();
      } catch (Exception e) {
   ...
   public void leaveTablClickedMethod() throws IOException {
      ...
      try {
         ...
         Type listType = new TypeToken<List<LeaveLogOfUserDto>>() {}.getType();
         ResponseData<LeaveLogOfUserDto> responseData = communicationUtils.jsonToResponseData(jsonReceivedStr, listType);
         String messageType = responseData.getMessageType();
   ...
   public void moneyTabClickedMethod() throws IOException {
      ...
      try {
         ...
         Type listType = new TypeToken<List<UserSalaryData>>() {}.getType();
         ResponseData<UserSalaryData> responseData = communicationUtils.jsonToResponseData(jsonReceivedStr, listType);
         String messageType = responseData.getMessageType();
